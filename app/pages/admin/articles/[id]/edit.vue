...
const { data: articleData, error: fetchError } = await useFetch(`/api/admin/articles/${articleId}`)
const articleRaw = computed(() => (articleData.value as any)?.article)
...
const form = reactive({
  title: articleRaw.value?.title || '',
  categoryId: String(articleRaw.value?.categoryId || ''),
  coverImage: articleRaw.value?.coverImage || '',
  excerpt: articleRaw.value?.excerpt || '',
  body: articleRaw.value?.body || '',
  status: (articleRaw.value?.status || 'draft') as 'draft' | 'published',
  tags: (articleRaw.value?.tags || []) as string[],
  animeIds: ((articleRaw.value?.anime || []).map((a: any) => a.id)) as number[],
})
